    ...
    for model_name in model_names:
        for loss_name in loss_names:
            ...
            if data_name == 'movielens':    
                candidates = np.array([]).reshape((0, 7))
                colnames = ['t', 'user', 'item', 'rating', 'A', 'F', 'D']
            elif data_name == 'lastfm':
                candidates = np.array([]).reshape((0, 6))
                colnames = ['t', 'user', 'item', 'A', 'F', 'D']
            ...
            for i in range(t):
                ...
                sub_candidates = np.hstack((np.repeat(i, exp.candidates.shape[0]).reshape((-1, 1)),
                                            exp.candidates.numpy(),
                                            exp.std_influence_value.numpy()))
                candidates = np.vstack((candidates, sub_candidates))
                
            # save
            pd.DataFrame(candidates, columns = colnames)\
                .to_csv("2_inference/augment/"+ data_name + '_' + model_name + '_' + loss_name +\
                        '.csv', index = False)
